...
from runner.CheckLastBackup import CheckLastBackup
from runner.Cleaner import Cleaner
from runner.DataSpaceChecker import DataSpaceChecker
from runner.InProgressTicket import (CheckInProgressTicket,
                                     SetInProgressTicket,
                                     UnsetInProgressTicket)
from runner.Mount import Mount
from runner.MoveBkpPeriod import MoveBkpPeriod
from runner.PgDump import PgDump
from runner.Rsync import Rsync
from runner.TarArchiver import TarArchiver
from runner.Umount import Umount
...
class ScheduleRunner:
    ...
    def __init__(self, task_name, verbose):
        self.logger = Logging.configure_logger(task_name, verbose)

        if not AppConfig.conf().schedule:
            self.logger.error('Отсутствует расписание в файле конфигурации')
            exit(-1)

        if not AppConfig.conf().schedule.get(task_name, {}):
            self.logger.error(
        ...
        ).schedule[task_name].get('actions')
    ...
    def create_action(self, action_name):
        ...
        for atype in map(lambda x: x[0], self.action_types):
            if isinstance(action, atypes[atype]):
                if not self.configure_action(atypes[atype], action,
                                             action_conf):
                    return None
