...
class TarArchiver(Action):
    ...
    def add_files_to_tar(self, src, tar):
        ...
        for (dirpath, _, filenames) in os.walk(src):
            ...
            try:
                mtime, fsize = os.path.getmtime(dirpath), os.path.getsize(
                    dirpath)
            except:
                self.logger.debug("Не удалось добавить %s", dirpath)
                continue
            ...
            if not exclude(dirpath, mtime, fsize):
                try:
                    ...
                    time.sleep(5)
                    self.logger.debug(
                        "Не удалось добавить, повторная попытка добавить файл: %s",
                        str(e),
                    )
                    try:
                        tar.addfile(dir)
                    except Exception as e:
                        self.logger.debug("Не удалось добавить файл: %s",
                                          str(e))
            ...
            for filename in filenames:
                ...
                try:
                    mtime, fsize = os.path.getmtime(filepath), os.path.getsize(
                        filepath)
                except:
                    self.logger.debug("Не удалось добавить %s", filepath)
                    continue
