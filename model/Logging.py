...
def get_web_module_logger_handler():
    ...
    log_filename = ".".join(["-".join([full_name, "web_module"]), "log"])

    web_log_path = os.path.join(get_log_path(), "web")
    if not os.path.exists(web_log_path):
        os.mkdir(web_log_path)

    date_format = AppConfig.get_log_dateformat()
    formatter = logging.Formatter(rec_format, date_format)
...
def configure_logger(task_name, verbose=False):
...
def get_logs_list():
    ...
    for dir in os.listdir(get_log_path()):
        if dir == "debug" or os.path.isfile(os.path.join(get_log_path(), dir)):
            continue
        res[dir] = []
        for filename in sorted(os.listdir(os.path.join(get_log_path(), dir)),
                               reverse=True):
            log = {}
            log["name"] = filename
            # log['debugname'] = '.'.join(['-'.join([filename.split('.')[0], 'debug']), 'log'])
...
def get_log_content(dir, filename):
    ...
    if len(lines) == 0:
        content[
            "lines"] = "файл отсутствует в файловой системе или нет прав на чтение"
    content["lines"] = lines
    return content
