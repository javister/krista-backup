...
Особенности: 1. Работает автономно 2. Имеет кросс-платформенную
конфигурацию 3. Логирование во время выполнения: сокращённое и
подробное 4. Интегрируема с системами мониторинга (например, Zabbix)
...
Документация в форматах html расположена и txt расположена в папке
docs.
...
1. Клонировать репозиторий. Место установки не имеет значения, но
обычно мы ставим приложение в каталог /opt. Для установки в /opt могут
потребоваться права sudo.

   cd /opt
   git clone ...

2. Установить зависимости для базовой системы (без веб-интерфейса) из
...
3. Установить python-зависимости для веб-интерфейса. Зависимости
устанавливаюстя с помощью утилиты pip из репо python. Для того чтобы
не менять окружение системы, лучше установить зависимости в тот же
каталог, что и приложение, испольуюя опцию --target, при запуске
установки из каталога приложения:
...
* Задания в конфиге сохраняются в разделе расписания (schedule).
