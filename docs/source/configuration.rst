...
- ``naming``
 ...
 Содержит ``server_name``, ``region`` и ``project``. Они используются для генерации выходящих файлов и
 логов. ``region`` и ``project``  могут быть пропущены, если их явно указать в запускаемом расписании.
